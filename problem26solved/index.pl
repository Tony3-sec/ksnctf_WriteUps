...
# Can you crack me? :P
open(F,'cracked.txt');
my $t = <F>; # read one line 		http://rfs.jp/sb/perl/02/08.html
chomp($t); # Delete \n (new line character) on last field
if ($t eq 'h@ck3d!') {
print 'FLAG_****************<br><br>';
}
unlink('cracked.txt');
...
open(F,substr($ENV{'PATH_INFO'},1)); # Delete slash in the head of URL path		http://perl.misty.ne.jp/21.html
                                       # like this "/foo/bar.txt" -> "foo/bar.txt"		http://r9.hateblo.jp/entry/20091206/p1
my $cgi = new CGI;
$cgi->charset('utf-8');
while(<F>) {
chomp;
